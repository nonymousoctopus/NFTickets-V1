...
export default function MarketScreen() {
  ...
  async function drawEventsGallery () {
    let tempEvents:any = [];
  ...
  return (
    ...
    <View style={styles.marketScreen}>
      ...
      <Text style={styles.heading}>Events on sale</Text>
      {loading ? (
      ...
      {events.map((event) => (
        <MarketItem key={event.itemId} itemId={event.itemId} name={event.name} price={event.price} image={event.image} tokenId={event.tokenId} available={event.available} start={event.start} location={event.location} startFull={event.startFull} finish={event.finish} description={event.description} nftContract={event.nftContract} />
      ))}
    </View>
    </ScrollView>
  );
