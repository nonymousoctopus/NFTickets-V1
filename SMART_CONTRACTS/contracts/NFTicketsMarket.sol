...
contract NFTicketsMarket is ReentrancyGuard, ERC1155Holder {
    ...
    function hostActions(uint256 _itemId, bytes memory _sig, address nftContract) public view returns (string memory) {
        string memory _message = idToMarketItem[_itemId].itemId; // This needs to be updated - need to stop using the name of the event as there could be conflicts - need to use the itemId from the marketItem
